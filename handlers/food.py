...
from database import SessionLocal, DailyLog, UserProduct, GlobalProduct
...
def handle_search(user_id, peer_id, text):
    ...
    try:
        ...
        if not products:
            send_message(peer_id, 
                ...
                get_main_keyboard())
            user_states[user_id] = {'state': 'ask_custom', 'query': clean_text, 'weight': weight}
            return
        
        user_states[user_id] = {'state': 'selecting', 'products': products, 'weight': weight}
        
        msg = f"Найденные продукты:\n\n"
...
def handle_selection(user_id, peer_id, product_name):
    ...
    if state['weight']:
        save_log(user_id, peer_id, selected, state['weight'])
    else:
        user_states[user_id] = {'state': 'weight', 'product': selected}
        send_message(peer_id, f"Выбрано: {selected['name']}\nВведите вес в граммах:", get_cancel_keyboard())
...
def handle_weight(user_id, peer_id, text):
    ...
    try:
        ...
        save_log(user_id, peer_id, user_states[user_id]['product'], weight)
    except ValueError:
...
def save_custom_product(user_id, peer_id, text):
    ...
    if state['state'] == 'custom_name':
        ...
        try:
            ...
            send_message(peer_id, f"✅ Продукт '{state['custom_name']}' сохранен!\n\nТеперь напишите его вес (например: 150г)", get_cancel_keyboard())
            user_states[user_id] = {'state': 'weight', 'product': {'name': state['custom_name'], 'calories': c, 'proteins': p, 'fats': f, 'carbs': u}}
...
def save_log(user_id, peer_id, product, weight):
    session = SessionLocal()
    try:
        m = weight / 100.0
        ...
        log = DailyLog(
            user_id=user_id, date=date.today(), product_name=product['name'], weight=weight,
